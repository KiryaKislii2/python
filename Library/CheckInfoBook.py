...
class CheckInfoBook:
    def __init__(self, id=0):
        ...
        self.year = input('ВВведите год:  ')
        self.sr_year()

        self.number = input('Введите артикул:  ')
        self.sr_number()
    ...
    def sr_number(self):
        while self.number.replace(' ', '') == '':
            print('\nERROR: Книга не может быть без артикула!')
            print('___________________________________________________________________________________')
            self.number = input('Введите артикул:  ')
        return self.number
    ...
    def result(self):
        return book(self.name, self.author, self.year, self.number, self.id)
